        ...
        if trimmed.contains("->") {
            ...
            if parts.len() >= 2 {
                ...
                let to = if let Some(bracket_pos) = to_part.find('[') {
                    to_part[..bracket_pos].trim().trim_matches('"').trim_end_matches(';')
                } else {
                    to_part.trim_matches('"').trim_end_matches(';')
                };
                
                // Ensure nodes exist
                let from_idx = *node_map.entry(from.to_string()).or_insert_with(|| {
                    let (node_type, level) = node_attributes.get(from)
                        .cloned()
                ...
                let to_idx = *node_map.entry(to.to_string()).or_insert_with(|| {
                    let (node_type, level) = node_attributes.get(to)
                        .cloned()
                        .unwrap_or((NodeType::Default, 0));
                    graph.add_node(NodeInfo {
                        label: to.to_string(),
...
fn setup(
    ...
    let dot_content = std::fs::read_to_string("hierarchy.dot")
    ...
    commands.spawn((
        Camera3d::default(),
        Transform::from_xyz(15.0, 10.0, 15.0).looking_at(Vec3::new(0.0, 4.0, 0.0), Vec3::Y),
        ...
        *level_idx += 1;
        ...
        let node_entity = commands.spawn((
            ...
            Transform::from_xyz(x, y, z)
                .with_scale(Vec3::splat(size_mult)),
            GraphNode,
        )).id();
        
        // Store the label in the Name component
        commands.entity(node_entity).insert(Name::new(node_info.label.clone()));
        ...
        commands.entity(node_entity).insert(LabelEntity(label_entity));
    }
    ...
    for edge in graph_data.graph.edge_indices() {
        if let Some((from_idx, to_idx)) = graph_data.graph.edge_endpoints(edge) {
            if let (Some(&from_pos), Some(&to_pos)) = (node_positions.get(&from_idx), node_positions.get(&to_idx)) {
                spawn_edge(&mut commands, &mut meshes, edge_material.clone(), from_pos, to_pos);
            }
        }
    }
...
fn camera_controls(
    ...
    for (mut transform, mut controller) in &mut query {
        ...
        if keyboard_input.pressed(KeyCode::PageUp) 
            ...
            controller.distance = (controller.distance - controller.speed * time.delta_secs()).max(5.0);
        }
        if keyboard_input.pressed(KeyCode::PageDown) 
            ...
            controller.distance = (controller.distance + controller.speed * time.delta_secs()).min(50.0);
        }
        ...
        if shift {
            ...
            if keyboard_input.pressed(KeyCode::ArrowUp) {
                controller.pitch_angle = (controller.pitch_angle - controller.rotation_speed * time.delta_secs())
                    .clamp(-std::f32::consts::PI / 3.0, std::f32::consts::PI / 3.0); // Limit to +/- 60 degrees
            }
